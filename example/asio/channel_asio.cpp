...
awaitable<void> produce_tokens(std::size_t bytes_per_token, steady_timer::duration token_interval, token_channel& tokens) {
    steady_timer timer(co_await this_coro::executor);
    for (;;) {
        co_await tokens.async_send(asio::error_code{}, bytes_per_token, use_awaitable);
...
awaitable<void> transfer(tcp::socket& from, tcp::socket& to, token_channel& tokens) {
    ...
    for (;;) {
        ...
        while (bytes_available > 0) {
            std::size_t n = co_await from.async_read_some(buffer(data, bytes_available), use_awaitable);
...
awaitable<void> proxy(tcp::socket client, tcp::endpoint target) {
    ...
    co_await (produce_tokens(bytes_per_token, token_interval, client_tokens) && transfer(client, server, client_tokens) && produce_tokens(bytes_per_token, token_interval, server_tokens) && transfer(server, client, server_tokens));
}
...
int main(int argc, char* argv[]) {
    try {
        ...
        auto listen_endpoint = *tcp::resolver(ctx).resolve(argv[1], argv[2], tcp::resolver::passive).begin();
